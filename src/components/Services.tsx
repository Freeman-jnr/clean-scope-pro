...
function ServiceCard({ service, index }: { service: ServiceCardProps; index: number }) {
  return (
    <div
      className="fade-up glow-ring group relative bg-white rounded-2xl border border-gray-200/80 card-hover hover:border-sky-light overflow-hidden"
    ...
    >
      <div className="relative h-44 overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-navy-deep/70 via-navy/10 to-transparent" />
        <div className="absolute top-3 left-3 w-11 h-11 glass-light rounded-xl flex items-center justify-center text-xl shadow-[0_8px_24px_rgba(2,132,199,0.18)]">
          {service.icon}
        </div>
      </div>
      <div className="p-7 pt-6">
        <h3 className="font-outfit font-bold text-navy text-lg mb-2 group-hover:text-sky-deeper transition-colors">
          {service.title}
        </h3>
        ...
        <ul className="space-y-2 mb-6 list-none">
          {service.checklistItems.map((item) => (
            <li key={item} className="flex items-start gap-2 text-sm text-gray-600">
              <span className="w-[18px] h-[18px] bg-gradient-to-br from-sky to-sky-deeper text-white text-xs font-bold rounded-full inline-flex items-center justify-center flex-shrink-0 mt-0.5">
                ✓
              </span>
              {item}
            </li>
...
export default function Services() {
  return (
    <section id="services" className="relative bg-gray-50 py-24 px-6 sm:px-8 overflow-hidden">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full bg-sky/15 blur-[120px] pointer-events-none" />
      <div className="relative max-w-6xl mx-auto">
        <div className="fade-up text-center max-w-2xl mx-auto mb-14">
          ...
          <h2 className="font-outfit font-extrabold text-3xl md:text-5xl text-navy leading-[1.1] tracking-[-0.025em] mb-4">
            Cleaning Services <span className="text-gradient-sky">Tailored to You</span>
          </h2>
